...
Supports Python, JavaScript, TypeScript with full
    method/class/import resolution.
...
def _extract_python_ts(file_path: str, content: str) -> ExtractionResult:
    ...
    def _process_class(node, parent_id: str, prefix: str = ""):
        ...
        node_id = _make_node_id(
            file_path, full_name, "class", start_line=node.start_point[0] + 1
        )

        # Decorators live on the parent decorated_definition,
        # not on class_definition
        decorators = []
        parent = node.parent
        if parent and parent.type == "decorated_definition":
            for child in parent.children:
                if child.type == "decorator":
                    decorators.append(
                        _get_node_text(child, source_bytes).lstrip("@")
                    )
                    ...
                    if stmt.type == "function_definition":
                        ...
                        for sub in stmt.children:
                            if sub.type == "function_definition":
                                _process_function(
                                    sub, node_id, prefix=f"{full_name}."
                                )
    ...
    def _process_function(node, parent_id: str, prefix: str = ""):
        ...
        kind = (
            "method"
            if parent_data and parent_data.kind == "class"
            else "function"
        )

        node_id = _make_node_id(
            file_path, full_name, kind, start_line=node.start_point[0] + 1
        )
            ...
            if child.type == "call":
                ...
                if func_node:
                    ...
                    if call_name not in (
                        ...
                        # Use the last part for method calls
                        # (e.g., self.foo -> foo)
                        target_name = (
                            call_name.split(".")[-1]
                            if "." in call_name
        ...
        if node.type == "import_statement":
            for child in node.children:
                if child.type == "dotted_name":
                    ...
                    result.edges.append(
                        ExtractedEdge(module_id, f"*::module::{mod}", "imports")
                    )
            ...
            for child in node.children:
                if child.type == "import_prefix":
                    continue
                if (
                    child.type in ("dotted_name", "identifier")
                    and child != node.children[1]
                ):
                ...
                if sub.type == "assignment":
                    ...
                    if left and left.type == "identifier":
                        ...
                        if var_name.isupper() or var_name.startswith("_"):
                            ...
                            result.edges.append(
                                ExtractedEdge(module_id, var_id, "defines")
                            )
    ...
    def _process_node(node, parent_id: str, prefix: str = ""):
        ...
        if node.type == "class_declaration":
            ...
            if name_node:
                ...
                result.edges.append(
                    ExtractedEdge(parent_id, node_id, "defines")
                )
            ...
            if name:
                ...
                params_node = node.child_by_field_name("parameters")
                sig = (
                    _get_node_text(params_node, source_bytes)
                    if params_node
                    else "()"
                )
                ...
                if child.type == "variable_declarator":
                    ...
                    if name_node:
                        ...
                        if value_node and value_node.type in (
                            ...
                            result.edges.append(
                                ExtractedEdge(parent_id, var_id, "defines")
                            )
                    ...
                    for child in body.children:
                        if child.type in (
                            ...
                            if sig_name:
                                ...
                                result.edges.append(
                                    ExtractedEdge(node_id, sid, "defines")
                                )
                    ...
                    for child in body.children:
                        if child.type == "enum_assignment":
                            ...
                            if mem_name:
                                ...
                                result.edges.append(
                                    ExtractedEdge(node_id, mid, "defines")
                                )
...
def extract_file_ts(
    ...
    """Extract structural elements using tree-sitter when available,
    regex as fallback.
    ...
    try:
        from codeloom.core.tags_extract import extract_file_tags
