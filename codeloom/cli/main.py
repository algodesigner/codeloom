"""CLI interface for codeloom.

Usage:
    codeloom build <source_dir> [--output <dir>] [--model <name>]
    codeloom search <query> [--db <path>] [--top-k <n>]
    codeloom stats [--db <path>]
    codeloom export [--db <path>] [--format json|graphml]
"""
...
def search(
    ...
    if not db_path:
        json_error("No knowledge base found. Run 'codeloom build' first.")
...
def search_graph(query: str, db: str | None, top_k: int, source_dir: str):
    ...
    from codeloom.query.embeddings import embed_query_dual
    from codeloom.query.hybrid import _weighted_expand
    from codeloom.storage.store import KnowledgeStore
...
def clean(source_dir: str, db: str | None, yes: bool):
    ...
    if db:
        ...
        kb_dir = Path(source_dir).resolve() / ".codeloom"
        if not kb_dir.exists():
            click.echo("No .codeloom/ directory found.")
            return
        if not yes:
...
def query(db: str | None, source_dir: str, top_k: int):
    ...
    while True:
        try:
            user_input = click.prompt("codeloom", prompt_suffix="> ")
...
def doctor():
    ...
    cwd = Path.cwd()
    db_path = cwd / ".codeloom" / "knowledge.db"
    if db_path.exists():
...
def mcp():
    ...
    Configure in Claude Code:
        ...
        claude mcp add codeloom -- codeloom mcp
