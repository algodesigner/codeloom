...
class TestOpenCodeIntegration:
    def test_opencode_install_project_scope(self, tmp_path):
        runner = CliRunner()
        with runner.isolated_filesystem(temp_dir=tmp_path):
            result = runner.invoke(cli, ["opencode", "install", "--scope", "project"])
            assert result.exit_code == 0
    ...
    def test_opencode_install_user_scope(self, tmp_path):
        runner = CliRunner()
        with runner.isolated_filesystem(temp_dir=tmp_path):
            result = runner.invoke(cli, ["opencode", "install", "--scope", "user"])
            assert result.exit_code == 0
            home = Path.home()
            skill_file = home / ".config" / "opencode" / "skills" / "codeloom" / "SKILL.md"
            assert skill_file.exists()
            assert "codeloom" in skill_file.read_text()
            # Clean up
            import shutil
    ...
    def test_opencode_install_idempotent(self, tmp_path):
        runner = CliRunner()
        with runner.isolated_filesystem(temp_dir=tmp_path):
            result1 = runner.invoke(cli, ["opencode", "install", "--scope", "project"])
            result2 = runner.invoke(cli, ["opencode", "install", "--scope", "project"])
    ...
    def test_opencode_uninstall_project_scope(self, tmp_path):
        ...
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            result = runner.invoke(cli, ["opencode", "uninstall", "--scope", "project"])
            assert result.exit_code == 0
...
class TestClaudeIntegration:
    ...
    def test_claude_uninstall_project(self, tmp_path):
        ...
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            result = runner.invoke(cli, ["claude", "uninstall", "--scope", "project"])
            assert result.exit_code == 0
...
class TestCodexIntegration:
    ...
    def test_codex_uninstall(self, tmp_path):
        ...
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            assert result.exit_code == 0
            agents_md = Path("AGENTS.md")
            assert not agents_md.exists() or "codeloom" not in agents_md.read_text()
...
class TestAiderIntegration:
    ...
    def test_aider_uninstall(self, tmp_path):
        ...
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            assert result.exit_code == 0
            conventions = Path("CONVENTIONS.md")
            assert not conventions.exists() or "codeloom" not in conventions.read_text()
