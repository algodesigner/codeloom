...
codeloom使用轻量级本地LLM模型，从10,000+文件的代码库和知识文档构建可查询的代码图和知识库。带子图响应的混合向量+关键词搜索（向量+关键词→基于MST子图的RRF融合）让编程代理真正理解你的整个项目，而不仅仅是搜索关键词。安装后Claude Code即可看到全貌——无需额外的token，无需额外的命令，一切100%本地运行。
...
cd your-project/
codeloom opencode install    # 适用于OpenCode
# 或: codeloom claude install  # 适用于Claude Code
```
...
codeloom通过一个命令与主要AI编程代理集成：

| 代理 | 安装 | 说明 |
|------|------|------|
| **Claude Code** | `codeloom claude install` | Skill + CLAUDE.md + PreToolUse钩子 |
| **OpenCode** | `codeloom opencode install` | `.opencode/skills/`中的技能 |
| **Codex CLI** | `codeloom codex install` | AGENTS.md + PreToolUse钩子 |
| **Gemini CLI** | `codeloom gemini install` | GEMINI.md + BeforeTool钩子 |
| **Cursor IDE** | `codeloom cursor install` | `.cursor/rules/`规则文件 |
| **Windsurf IDE** | `codeloom windsurf install` | `.windsurf/rules/`规则文件 |
| **Cline** | `codeloom cline install` | `.clinerules`文件 |
| **Aider CLI** | `codeloom aider install` | CONVENTIONS.md + `.aider.conf.yml` |
| **MCP服务器** | `claude mcp add codeloom -- codeloom mcp` | Model Context Protocol 5个工具 |

每个`install`会写入上下文文件，并（在支持的平台上）注册工具调用前的钩子。卸载：`codeloom <platform> uninstall`。

## 支持的语言

### 结构提取（20+种语言）

codeloom使用tree-sitter和原生解析器提取函数、类、方法、调用、import、继承。
...
与AI编码代理（Claude Code、Codex等）集成时，codeloom会在代码变更时**自动重建图**。Stop/SessionEnd钩子通过`git diff`检测变更文件，并在后台执行增量构建——无需手动操作。
...
在codeloom自身代码库上的基准测试（约3,500行，90个文件，1,300个节点）：
...
- **嵌入模型**: ~180MB，仅下载一次到`~/.codeloom/models/`
