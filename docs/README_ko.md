...
codeloom는 10,000개 이상의 파일을 가진 코드베이스와 지식 문서들로부터, 경량 로컬 LLM 모델을 사용하여 질의 가능한 코드 그래프와 지식 베이스를 구축합니다. 서브그래프 응답을 포함한 하이브리드 벡터 + 키워드 검색(벡터 + 키워드 → MST 서브그래프를 통한 RRF 퓨전)으로 코딩 에이전트가 프로젝트 전체를 진정으로 이해할 수 있게 됩니다. 설치하면 Claude Code가 전체 그림을 볼 수 있습니다 — 추가적인 토큰도, 추가적인 명령도 필요 없으며, 모든 것이 100% 로컬에서 실행됩니다.
...
cd your-project/
codeloom opencode install    # OpenCode의 경우
# 또는: codeloom claude install  # Claude Code의 경우
```
...
codeloom는 주요 AI 코딩 에이전트와 한 명령어로 통합됩니다:

| 에이전트 | 설치 | 설명 |
|---------|------|------|
| **Claude Code** | `codeloom claude install` | Skill + CLAUDE.md + PreToolUse 훅 |
| **OpenCode** | `codeloom opencode install` | `.opencode/skills/`에 스킬 |
| **Codex CLI** | `codeloom codex install` | AGENTS.md + PreToolUse 훅 |
| **Gemini CLI** | `codeloom gemini install` | GEMINI.md + BeforeTool 훅 |
| **Cursor IDE** | `codeloom cursor install` | `.cursor/rules/` 규칙 파일 |
| **Windsurf IDE** | `codeloom windsurf install` | `.windsurf/rules/` 규칙 파일 |
| **Cline** | `codeloom cline install` | `.clinerules` 파일 |
| **Aider CLI** | `codeloom aider install` | CONVENTIONS.md + `.aider.conf.yml` |
| **MCP 서버** | `claude mcp add codeloom -- codeloom mcp` | Model Context Protocol 5개 도구 |

각 `install`은 컨텍스트 파일 작성과 (지원하는 플랫폼의 경우) 도구 호출 전 훅 등록을 수행합니다. 제거: `codeloom <platform> uninstall`.

## 지원 언어

### 구조 추출 (20개 이상 언어)

codeloom는 tree-sitter와 네이티브 파서를 사용하여 함수, 클래스, 메서드, 호출, import, 상속을 추출합니다.
...
AI 코딩 에이전트(Claude Code, Codex 등)와 통합 시, codeloom는 코드 변경 시 **자동으로 그래프를 리빌드**합니다. Stop/SessionEnd 훅이 `git diff`로 변경된 파일을 감지하고 백그라운드에서 증분 빌드를 실행합니다 — 수동 작업이 필요 없습니다.
...
| `.codeloom-ignore` | 코드 그래프 전용 프로젝트별 오버라이드 |
...
codeloom 자체 코드베이스 기준 벤치마크 (~3,500줄, 90개 파일, 1,300개 노드):
...
- **임베딩 모델**: ~180MB, `~/.codeloom/models/`에 한 번만 다운로드
